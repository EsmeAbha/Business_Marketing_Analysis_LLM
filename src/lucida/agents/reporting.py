...
"""
    ...
    def _run_footer(self, session_id: str, financials: dict) -> str:
        """Nothing. The owner is a shopkeeper, not an operator.

        This used to sign every report with the session id, the number of
        model calls, the token count and the cost. That belongs on the
        Workforce screen, which already shows it — in the middle of a
        business report it is noise, and it was reaching the owner with the
        markdown backticks still showing.
        """
        return ""
